...
function Task(props: {data: DataState, info: TaskInfo}) {
  const {
    email, id, image_path, status, text, username,
  } = props.info;
  const {
  ...
  const handleSave = () => {
    ...
    patchTask({
      id, developerName, token, text: textInput, status: parseStatus(isDone, newEdited),
    })
      .then(refreshData);
